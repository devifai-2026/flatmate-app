 ...
 * Hero image, PG-specific fields (sharing, gender, meals), amenities, host card, contact CTA.
 */
...
const GENDER_LABELS: Record<string, { label: string; color: string }> = {
  ...
  female: { label: 'Female only', color: colors.primary },
  ...
  if (!pg) {
    return (
      <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
        ...
        <View style={styles.loader}>
          <Icon name="business-outline" size={52} color={colors.gray300} />
          <Text style={styles.notFoundText}>PG not found</Text>
        </View>
      </SafeAreaView>
    );
  }

  const initials = pg.postedBy?.name
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.hero}>
          {pg.images?.[0] ? (
            ...
            <View style={styles.heroPlaceholder}>
              <Icon name="business-outline" size={64} color={colors.gray300} />
            </View>
          )}
          <LinearGradient
          ...
          <View style={styles.availBadge}>
            ...
            <Text style={styles.availText}>Available from {formatDate(pg.availableFrom)}</Text>
          </View>
        </View>
        ...
        <View style={styles.body}>
          ...
          <View style={styles.badgeRow}>
            ...
            {genderMeta && (
              <View style={[styles.genderBadge, { backgroundColor: `${genderMeta.color}18` }]}>
                <Text style={[styles.genderBadgeText, { color: genderMeta.color }]}>
                  {genderMeta.label}
                </Text>
              </View>
            )}
            {pg.meals && (
              <View style={styles.mealsBadge}>
                ...
                <Text style={styles.mealsBadgeText}>Meals included</Text>
              </View>
            )}
          </View>
          ...
          <View style={styles.infoGrid}>
            <View style={styles.infoBox}>
              ...
              <Text style={styles.infoBoxValue}>₹{pg.rent.toLocaleString()}</Text>
            </View>
            {pg.deposit && (
              <View style={styles.infoBox}>
                <Text style={styles.infoBoxLabel}>Deposit</Text>
                <Text style={styles.infoBoxValueSm}>₹{pg.deposit.toLocaleString()}</Text>
              </View>
            )}
            {pg.sharing && (
              <View style={styles.infoBox}>
                <Text style={styles.infoBoxLabel}>Sharing</Text>
                <Text style={styles.infoBoxValueSm}>
                  {SHARING_LABELS[pg.sharing] ?? pg.sharing}
                </Text>
              </View>
            )}
          </View>
          ...
          {pg.city && (
            <View style={styles.section}>
              ...
              <View style={styles.locationRow}>
                <Icon name="map-outline" size={15} color={colors.muted} />
            ...
            <View style={styles.hostCard}>
              <View style={styles.hostAvatarWrap}>
                {pg.postedBy?.profileImage ? (
                  <Image source={{ uri: pg.postedBy.profileImage }} style={styles.hostAvatar} />
                ) : (
              ...
              <View style={styles.hostInfo}>
                ...
                {pg.createdAt && (
                  <Text style={styles.hostPosted}>Posted {formatPostedAgo(pg.createdAt)}</Text>
                )}
              </View>
            </View>
          </View>
          ...
          <TouchableOpacity style={styles.ctaBtn} onPress={handleContact} activeOpacity={0.88}>
            ...
            >
              ...
              <Text style={styles.ctaBtnText}>
                {pg.phoneVisibility === 'masked' ? 'View Contact • ₹19' : 'View Contact'}
              </Text>
            </LinearGradient>
          </TouchableOpacity>
        )}
...
const styles = StyleSheet.create({
  ...
  mealsBadgeText: { fontSize: 11, fontWeight: '700', color: colors.success },
  ...
  infoBoxValueSm: { ...textPresets.label, color: colors.dark },
